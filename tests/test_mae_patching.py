...
from seis_attr_ssl.models.mae.patching import (
	compute_num_patches,
	patchify_3d,
	unpatchify_3d,
)
...
def test_patchify_3d_rejects_invalid_tensor_rank() -> None:
	x = torch.zeros((1, 1, 8, 12))

	with pytest.raises(ValueError, match=r'shape=\(1, 1, 8, 12\)'):
		patchify_3d(x, (2, 3, 4))


def test_unpatchify_3d_rejects_invalid_tensor_rank() -> None:
	patches = torch.zeros((1, 4, 8))

	with pytest.raises(ValueError, match=r'shape=\(1, 4, 8\)'):
