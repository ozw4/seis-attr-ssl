...
class EmbeddingMerger:
	...
	def add_window(
		...
		window_token_shape = cast(
			'XYZ',
			tuple(
				size_axis // patch_axis
				for size_axis, patch_axis in zip(window.size_xyz, patch, strict=True)
			),
		)
		...
		if embeddings.shape != expected_embedding_shape:
			msg = (
				...
				f'embedding_dim; got {embeddings.shape!r}, '
				f'expected={expected_embedding_shape!r}'
			)
			raise ValueError(msg)
		valid = np.asarray(token_valid_mask, dtype=bool)
		if valid.shape != window_token_shape:
			msg = (
				'token_valid_mask shape must match window token grid; got '
				f'{valid.shape!r}, expected={window_token_shape!r}'
			)
			raise ValueError(msg)
		if not valid.any():
			return
	...
	def finalize(
		...
		with np.errstate(divide='ignore', invalid='ignore'):
			output = self.sums / np.maximum(self.counts[..., np.newaxis], 1)
		averaged[valid] = output[valid].astype(dtype, copy=False)
	...
	def write_average(
		...
		for x_index in range(self.token_grid_shape_xyz[0]):
			...
			out[valid] = (
				self.sums[x_index][valid] / counts[valid, np.newaxis]
			).astype(dtype, copy=False)
			embeddings[x_index] = out
...
def _merge_slices(
	...
	for start, size, limit in zip(
		...
		if clipped_stop <= clipped_start:
			...
			raise ValueError(msg)
		target_slices.append(slice(clipped_start, clipped_stop))
		source_slices.append(slice(clipped_start - start, clipped_stop - start))
	return (
		cast('tuple[slice, slice, slice]', tuple(target_slices)),
		cast('tuple[slice, slice, slice]', tuple(source_slices)),
	)
...
def _validate_array(
	...
	if array.dtype != dtype:
		msg = f'{name} dtype must be {dtype.name}; got {array.dtype.name}.'
		raise TypeError(msg)
	return array
