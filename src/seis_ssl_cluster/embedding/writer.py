...
def output_paths(output_dir: str | Path, survey_id: str) -> EmbeddingOutputPaths:
	...
	return EmbeddingOutputPaths(
		root / f'{survey_id}.embeddings.npy',
		root / f'{survey_id}.valid_tokens.npy',
		root / f'{survey_id}.embedding_metadata.json',
		root / f'.{survey_id}.embeddings.tmp.npy',
		root / f'.{survey_id}.valid_tokens.tmp.npy',
		root / f'.{survey_id}.embedding_sum.float32.npy',
		root / f'.{survey_id}.embedding_count.uint32.npy',
	)
...
def commit_staged_outputs(
	...
	for staged_path in (paths.embeddings_tmp, paths.valid_tokens_tmp):
		if not staged_path.is_file():
			msg = f'missing staged embedding output: {staged_path}'
...
def metadata_matches(path: Path, metadata: dict[str, object]) -> bool:
