	...
	for x_start in starts_by_axis[0]:
		for y_start in starts_by_axis[1]:
			for z_start in starts_by_axis[2]:
				yield SlidingWindow((x_start, y_start, z_start), window)
...
def _validate_window_geometry(
	...
	stride_xyz = compute_stride_xyz(window_size_xyz, overlap_xyz)
	for name, values in (
		...
		if any(
			value % patch_axis != 0
			for value, patch_axis in zip(values, patch_size_xyz, strict=True)
		):
			msg = (
				f'{name} must align to patch_size_xyz; got {name}={values!r}, '
				f'patch_size_xyz={patch_size_xyz!r}'
			)
			raise ValueError(msg)


def _axis_starts(padded_size: int, window_size: int, stride: int) -> tuple[int, ...]:
