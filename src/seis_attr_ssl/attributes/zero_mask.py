"""Zero-amplitude invalid-mask utilities for seismic attribute generation."""
...
class ZeroAmplitudeMaskConfig:
	"""Configuration for deterministic zero-amplitude invalid masking."""

	enabled: bool = True
	zero_atol: float = 0.0
	z_sample_influence_radius: int = 64
	xy_trace_influence_radius: int = 1
	z_trace_influence_radius: int = 0

	def validate(self) -> None:
		"""Validate zero-amplitude mask settings."""
		if not isinstance(self.enabled, bool):
			msg = f'enabled must be a bool; got {self.enabled!r}'
			raise TypeError(msg)
		if not isinstance(self.zero_atol, Real) or self.zero_atol < 0.0:
			msg = (
				'zero_atol must be a non-negative real number; '
				f'got {self.zero_atol!r}'
			)
		...
		_validate_nonnegative_int(
			self.z_trace_influence_radius,
			'z_trace_influence_radius',
		)


DEFAULT_ZERO_AMPLITUDE_MASK_CONFIG = ZeroAmplitudeMaskConfig()
...
def compute_zero_amplitude_invalid_mask(
	...
	*,
	...
	config: ZeroAmplitudeMaskConfig = DEFAULT_ZERO_AMPLITUDE_MASK_CONFIG,
) -> np.ndarray:
	"""Return a boolean [x, y, z] invalid mask for zero regions and influence."""
	...
	no_valid_z_samples = ~np.any(valid, axis=(0, 1))
	invalid[:, :, no_valid_z_samples] = True
	...
	invalid |= dilate_zero_trace_mask(
		...
		radius_z=config.z_trace_influence_radius,
	)
	no_valid_traces = ~np.any(valid, axis=2)
	invalid[no_valid_traces, :] = True
...
def detect_all_zero_z_samples(
	...
	*,
	...
	"""Return [z] flags where every valid voxel at z is zero-like."""
	amplitude, valid = _prepare_amplitude_and_valid_mask(amplitude_xyz, valid_mask)
...
def detect_all_zero_traces(
	...
	*,
	...
	"""Return [x, y] flags where every valid voxel along a trace is zero-like."""
	amplitude, valid = _prepare_amplitude_and_valid_mask(amplitude_xyz, valid_mask)
	_validate_zero_atol(zero_atol)
	zero_like = np.abs(amplitude) <= np.float32(zero_atol)
...
def dilate_zero_trace_mask(
	...
	*,
	...
	_validate_nonnegative_int(radius_z, 'radius_z')
...
def _validate_zero_atol(zero_atol: float) -> None:
	if not isinstance(zero_atol, Real) or zero_atol < 0.0:
		msg = f'zero_atol must be a non-negative real number; got {zero_atol!r}'
		raise ValueError(msg)


__all__ = [
	'DEFAULT_ZERO_AMPLITUDE_MASK_CONFIG',
