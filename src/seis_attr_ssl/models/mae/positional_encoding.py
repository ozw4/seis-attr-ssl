"""3D positional encoding helpers for strict MAE token grids."""
...
def restore_decoder_sequence(
	...
	Returns ``(decoder_tokens, masked_token_mask)`` where ``decoder_tokens`` has
	shape ``[B, N, D]`` and already includes ``pos`` at every spatial position.
	``masked_token_mask`` has shape ``[B, N]`` with ``True`` for masked tokens.
	"""
