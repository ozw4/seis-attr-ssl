...
def inspect_npy_volume(path: str | Path) -> NpyVolumeInfo:
	"""Inspect a numeric 3D `.npy` volume using NumPy's `.npy` memmap loader."""
...
class NpyMemmapVolumeStore:
	...
	Padded crops preserve the volume dtype. The scalar ``pad_value`` is converted
	to that dtype when the output array is allocated.
	"""
...
__all__ = ['NpyMemmapVolumeStore', 'NpyVolumeInfo', 'inspect_npy_volume']
