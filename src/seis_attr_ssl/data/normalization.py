"""Survey-wise robust normalization for base seismic amplitude volumes."""
...
from seis_attr_ssl.data.schema import GRID_ORDER_XYZ
...
class SurveyNormalizationStats:
	...
	def validate(self) -> None:
		"""Validate the fixed MVP survey-wise normalization contract."""
...
def write_normalization_stats(
	...
	"""Write survey-level normalization statistics as JSON."""
	stats.validate()
	stats_path = Path(path)
...
def normalize_amplitude(
	...
	"""Clip and robust-scale an amplitude crop without changing XYZ shape/order."""
	stats.validate()
...
def compute_normalization_stats(  # noqa: PLR0913
	...
	*,
	...
	When ``max_samples`` is smaller than the number of voxels, a deterministic
	voxel sample is used instead of materializing the full survey in memory.
	"""
