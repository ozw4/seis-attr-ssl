"""Build NOPIMS manifests from configured `.npy` attribute volumes."""
...
DEFAULT_OUTPUT_NAME = 'nopims_manifests.json'
...
def main() -> None:
	...
	if args.dry_run:
		...
		_print_base_seismic_warning(manifest_cfg)
		return

	result = scan_nopims_manifests(nopims_root, scan_pattern)
...
def _print_base_seismic_warning(manifest_cfg: Mapping[str, object]) -> None:
	if manifest_cfg.get('require_base_seismic') is True:
		print(
			'warning: manifest.require_base_seismic is currently advisory; '
			'no base seismic record is written.'
		)
